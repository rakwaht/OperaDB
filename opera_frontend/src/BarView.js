                ...
                return response.json() })   
                    .then( (json) => {
                    	...
                    	(json||[]).forEach(function(obj){ 
				    		if(obj && obj['mensa']==='Bar Polo Tecnologico'){ 
                    			let week_number = parseInt(obj['week']);
                    			let key = (week_number - firstWeek).toString();
                    			data['povo'][key]['value'] = obj;
				    		}
				    		else if(obj && obj['mensa']==='Bar Mesiano'){ 
                    			let week_number = parseInt(obj['week']);
                    			let key = (week_number - firstWeek).toString();
                    			data['mesiano'][key]['value'] = obj;
				    		}
				    		else if(obj && obj['mensa']==='Bar S.Bartolomeo'){ 
                    			let week_number = parseInt(obj['week']);
                    			let key = (week_number - firstWeek).toString();
                    			data['samba'][key]['value'] = obj;
				    		}
						});
        	...
        	toBeRendered = (
			  <div className="row">
			    <div className="col-sm-4">
			    	<SingleCanteen 
			    		canteenName="Bar Povo"
			    		data={this.state.data['povo']}
			    	/>
			    </div>
			    <div className="col-sm-4">
			    	<SingleCanteen 
			    		canteenName="Bar Mesiano"
			    		data={this.state.data['mesiano']}
			    	/>
			    </div>
			    <div className="col-sm-4">
			    	<SingleCanteen 
			    		canteenName="Bar Samba"
			    		data={this.state.data['samba']}
			    	/>
			    </div>
			  </div>
    		);
