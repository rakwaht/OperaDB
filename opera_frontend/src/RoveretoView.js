...
class RoveretoView extends React.Component {
                ...
                return response.json() })   
                    .then( (json) => {
                    	...
                    	(json||[]).forEach(function(obj){ 
				    		if(obj && obj['mensa']==='Mensa Mart Rovereto'){ 
                    			let week_number = parseInt(obj['week']);
                    			let key = (week_number - firstWeek).toString();
                    			data['mart'][key]['value'] = obj;
				    		}
				    		else if(obj && obj['mensa']==='Mensa Gilda Rovereto'){ 
                    			let week_number = parseInt(obj['week']);
                    			let key = (week_number - firstWeek).toString();
                    			data['gilda'][key]['value'] = obj;
				    		}
						});
        	...
        	toBeRendered = (
			  <div className="row">
			    <div className="col-sm-6">
			    	<SingleCanteen 
			    		canteenName="Mensa Mart Rovereto"
			    		data={this.state.data['mart']}
			    	/>
			    </div>
			    <div className="col-sm-6">
			    	<SingleCanteen 
			    		canteenName="Mensa Gilda Rovereto"
			    		data={this.state.data['gilda']}
			    	/>
			    </div>
			  </div>
    		);
